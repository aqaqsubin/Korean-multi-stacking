...
from others.utils import clean, del_folder, mkdir_p
...
def tokenize(args):
    ...
    stories = list(iglob(pjoin(stories_dir, "**.story"),recursive=False))
    check_ = list(iglob(pjoin(tokenized_stories_dir, "**.json"),recursive=False))

    if len(check_) == 0 or len(check_) != len(list(stories)):
        del_folder(tokenized_stories_dir)
        mkdir_p(tokenized_stories_dir)
...
def format_to_lines(args):
    ...
    for corpus_type in ['test', 'valid', 'train']:
        temp = []
        with open(pjoin(args.map_path, corpus_type + '_data_idx.txt'), 'rb') as f:
            temp = pickle.load(f)
